...
import com.github.davenury.ucac.commitment.gpac.ElectedYou
import com.github.davenury.ucac.commitment.gpac.GPACProtocolClient
...
class GPACProtocolSpec {
    ...
    @Test
    fun `should return elected you, when ballot number is lower than proposed`(): Unit =
        runBlocking {
    ...
    @Test
    fun `should throw NotElectingYou when ballot number is higher than proposed`(): Unit =
        runBlocking {
            coEvery { transactionBlockerMock.getAcquisition() } returns null
            coEvery { transactionBlockerMock.acquireReentrant(TransactionAcquisition(ProtocolName.GPAC, change.id)) } just Runs
            coEvery { transactionBlockerMock.tryRelease(TransactionAcquisition(ProtocolName.GPAC, change.id)) } returns true
    ...
    @Test
    fun `should return elected you with commit init val, when history can be built`(): Unit =
        runBlocking {
            coEvery { transactionBlockerMock.getAcquisition() } returns null
            coEvery { transactionBlockerMock.acquireReentrant(TransactionAcquisition(ProtocolName.GPAC, change.id)) } just Runs
            coEvery { transactionBlockerMock.tryRelease(TransactionAcquisition(ProtocolName.GPAC, change.id)) } returns true
    ...
    @Test
    fun `should change ballot number and return agreed, when asked to ft-agree on change`(): Unit =
        runBlocking {
            coEvery { transactionBlockerMock.getAcquisition() } returns null
            coEvery { transactionBlockerMock.acquireReentrant(TransactionAcquisition(ProtocolName.GPAC, change.id)) } just Runs
            coEvery { transactionBlockerMock.tryRelease(TransactionAcquisition(ProtocolName.GPAC, change.id)) } returns true
    ...
    @Test
    fun `should throw not electing you, when proposed ballot number is less than state's`(): Unit =
        runBlocking {
    ...
    fun `should apply change`(): Unit =
        runBlocking {
            ...
            every {
                transactionBlockerMock.release(
    ...
    @Test
    fun `should not apply change when acceptVal is abort`(): Unit =
        runBlocking {
            coEvery { transactionBlockerMock.getAcquisition() } returns null
            every { transactionBlockerMock.acquireReentrant(TransactionAcquisition(ProtocolName.GPAC, change.id)) } just Runs
            every { transactionBlockerMock.tryRelease(TransactionAcquisition(ProtocolName.GPAC, change.id)) } returns true
