...
import com.github.davenury.common.history.HistoryException
...
internal class HistoryTest {
    ...
    @Test
    fun `add the same entry multiple times`(): Unit =
        runBlocking {
            val parentId = history.getCurrentEntryId()
    ...
    @Test
    fun `get entry from history`(): Unit =
        runBlocking {
            val parentId = history.getCurrentEntryId()
            val entry1 = IntermediateHistoryEntry("test1", parentId)
    ...
    @Test
    fun `serialize initial entry`(): Unit =
        runBlocking {
    ...
    @Test
    fun `deserialize entry with wrong formatting`(): Unit =
        runBlocking {
    ...
    @Test
    fun `deserialize entry with wrong order`(): Unit =
        runBlocking {
            expectThrows<IllegalArgumentException> {
    ...
    @Test
    fun `id generation`(): Unit =
        runBlocking {
            assert(
                InitialHistoryEntry.getId() == "27c74670adb75075fad058d5ceaf7b20c4e7786c83bae8a32f626f9782af34c9a33c2046ef60fd2a7878d378e29fec851806bbd9a67878f3a9f1cda4830763fd",
            )
        }
}
