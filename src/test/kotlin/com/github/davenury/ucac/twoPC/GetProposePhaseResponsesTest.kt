...
class GetProposePhaseResponsesTest {
    ...
    @Test
    fun `should be able to work correctly with no redirects`(): Unit =
        runBlocking {
            coEvery { protocolClientMock.sendAccept(any(), any()) } returns
                mapOf(
    ...
    @Test
    fun `should repeat the accept only to peersets that returned redirects`(): Unit =
        runBlocking {
            coEvery { protocolClientMock.sendAccept(any(), any()) } returnsMany
                listOf(
                    // first return
                    mapOf(
                        peerResolver.resolve(PeerId("peer3")) to
                            TwoPCRequestResponse(
                                success = false,
                                redirect = true,
                                newConsensusLeaderId = PeerId("peer2"),
                                newConsensusLeaderPeersetId = PeersetId("peerset1"),
                                peersetId = PeersetId("peerset1"),
                            ),
                        peerResolver.resolve(PeerId("peer4")) to
                            TwoPCRequestResponse(
                                success = true,
                                peersetId = PeersetId("peerset2"),
                            ),
                    ),
                    // second return
                    mapOf(
            ...
            expectThat(
                subject.currentConsensusLeaders[PeersetId("peerset1")],
            ).isEqualTo(peerResolver.resolve(PeerId("peer2")))

            // and the calls should be valid
            coVerify(exactly = 1) {
                protocolClientMock.sendAccept(
                    mapOf(
                        PeersetId("peerset1") to peerResolver.resolve(PeerId("peer3")),
                        PeersetId("peerset2") to peerResolver.resolve(PeerId("peer4")),
                    ),
    ...
    @Test
    fun `should respect decision false of recent responses`(): Unit =
        runBlocking {
            coEvery { protocolClientMock.sendAccept(any(), any()) } returnsMany
                listOf(
                    // first return
