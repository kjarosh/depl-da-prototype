...
class TestApplicationSet(
    ...
    init {
        ...
        val testConfigOverrides =
            mapOf(
                "peers" to peerIds.joinToString(";") { peerId -> "$peerId=${NON_RUNNING_PEER}" },
                "peersets" to
                    peersetConfiguration.entries.joinToString(";") { (peersetId, peers) ->
        ...
        peerAddresses =
            apps.mapValues { (peerId, app) ->
                val address =
                    if (peerId.toString() in appsToExclude) {
                        NON_RUNNING_PEER
                    } else {
    ...
    fun getPeerAddresses(peersetId: PeersetId): Map<PeerId, PeerAddress> =
        peersets[peersetId]!!.associateWith { peerId ->
            getPeer(
                peerId,
            )
        }
    ...
    fun getRunningApps(): List<ApplicationUcac> =
        apps
