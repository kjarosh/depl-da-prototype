...
class GPACFactory(
    ...
    suspend fun handleElect(message: ElectMe) = getOrCreateGPAC(message.change.id).handleElect(message)

    suspend fun handleAgree(message: Agree) = getOrCreateGPAC(message.change.id).handleAgree(message)

    suspend fun handleApply(message: Apply) {
        getOrCreateGPAC(message.change.id).handleApply(message)
    }

    fun getChangeStatus(changeId: String) = changeIdToGpacInstance[changeId]?.getChangeResult(changeId)
}
