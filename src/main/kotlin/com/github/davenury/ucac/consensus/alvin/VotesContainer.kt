...
class VotesContainer {
    ...
    suspend fun initializeEntry(entryId: String) =
        mutex.withLock {
    ...
    suspend fun getVotes(
        entryId: String,
        isVotingForCommit: Boolean,
    ): Pair<Int, Int> =
        mutex.withLock {
            initializeEntryIfAbsent(entryId)
            val (commitModifier, abortModifier) = if (isVotingForCommit) Pair(0, -1) else Pair(-1, 0)
            Pair(
                entryIdToVotesCommit[entryId]!!.size + commitModifier,
                entryIdToVotesAbort[entryId]!!.size + abortModifier,
            )
        }
