...
class ChangesTest {
        ...
        runBlocking {
            ...
            val subject =
                Changes(
                    ...
                    DefaultChangeStrategy(OWN_ADDRESS),
                    null,
                    "",
                )
    ...
    @Test
    fun `should be able to unlock peersets`(): Unit =
        runBlocking {
    ...
    @Test
    fun `should be able to execute changes without any errors and with valid parents ids`(): Unit =
        runBlocking {
    ...
    fun `handle notification should be idempotent`() {
        ...
        val changes =
            Changes(
                peers,
                DummySender(shouldNotify = false, peers),
                strategy,
                DefaultChangeStrategy(
                    OWN_ADDRESS,
                ),
        ...
        changes.forEach { (_, change) ->
            change.peersets.forEach { (peersetId, parentId) ->
                if (parentId != mapOfChanges[peersetId]) {
                    println(
                        "ParentId differs for change in peerset $peersetId - parentId: $parentId, " +
                            "expected parent id: ${mapOfChanges[peersetId]}",
                    )
                    return false
                }
