...
class MeteredHistory(
    ...
    override fun getCurrentEntryId(): String = meterRegistry.timer("history_get_current_entry_id").record<String> { delegate.getCurrentEntryId() }!!
    ...
    override fun getEntry(id: String): HistoryEntry = meterRegistry.timer("history_get_entry").record<HistoryEntry> { delegate.getEntry(id) }!!

    override fun getEntryFromHistory(id: String): HistoryEntry? =
        meterRegistry.timer(
            "history_get_entry_from_history",
        ).record<HistoryEntry?> { delegate.getEntryFromHistory(id) }

    override fun toEntryList(skipInitial: Boolean): List<HistoryEntry> =
        meterRegistry.timer(
            "history_to_entry_list",
        ).record<List<HistoryEntry>> { delegate.toEntryList(skipInitial) }!!

    override fun containsEntry(entryId: String): Boolean = meterRegistry.timer("history_contains_entry").record<Boolean> { delegate.containsEntry(entryId) }!!

    override fun isEntryCompatible(entry: HistoryEntry): Boolean = meterRegistry.timer("history_is_entry_compatible").record<Boolean> { delegate.isEntryCompatible(entry) }!!
